...
const DailyPnlChart = () => {
  ...
  const [series, setSeries] = useState([{ name: 'Daily P&L', data: [] }]);
  ...
  useEffect(() => {
    ...
    const pnlData = data?.TotalNetPnl;
    setSeries([{ name: 'Daily P&L', data: Array.isArray(pnlData) && pnlData.length > 0 ? pnlData : [] }]);
