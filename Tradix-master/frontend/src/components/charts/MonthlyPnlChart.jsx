...
const MonthlyPnlChartOptions = {
    chart: { type: 'bar', height: '140px', fontFamily: 'Inter, sans-serif', toolbar: { show: false } },
    ...
    fill: { opacity: 1 },
    legend: {
        show: true, fontFamily: `'Public Sans', sans-serif`,
        offsetX: 10, offsetY: 10,
        labels: { useSeriesColors: false },
        markers: { width: 16, height: 16, offsexX: 2, offsexY: 2 },
        itemMargin: { horizontal: 15, vertical: 30 }
    },
};
...
const MonthlyPnlChart = () => {
    ...
    useEffect(() => {
        setOptions((prevState) => ({
            ...
            legend: { labels: { colors: currentMode === 'light' ? '#111827' : '#9ca3af' } },
            tooltip: { y: { formatter(val) { return `${currency + val}`; } } },
        }));
        setSeries([
            { name: 'Net Profit', data: Array.isArray(data?.TotalNetPnL) && data.TotalNetPnL.length > 0 ? data.TotalNetPnL : [] },
            { name: 'Revenue', data: Array.isArray(data?.TotalRevenue) && data.TotalRevenue.length > 0 ? data.TotalRevenue : [] },
        ]);
